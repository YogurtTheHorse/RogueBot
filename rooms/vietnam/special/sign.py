...
name = locale_manager.get('rooms.vietnam.special.sign.phrase_1')
	...
	for room_type, room_name in rooms:
		...
		if loaded_room.name == text or crypt(loaded_room.name) == text:
			if random.random() < 0.1:
				reply(locale_manager.get('rooms.vietnam.special.sign.phrase_2'))
				user.open_room(reply)
			else:
				user.open_room(reply, room_type, room_name)
			return

	reply(locale_manager.get('rooms.vietnam.special.sign.phrase_3'))
