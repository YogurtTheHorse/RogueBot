...
def death(self, reply, reason=None):
	if self.state == 'room':
		...
		reply(locale_manager.get(locale_manager.get('messages.dead_place', self.lang)).format(room.name))
	...
	reply(locale_manager.get('messages.dead_message', self.lang).format(self.monsters_killed, self.rooms_count), [ '/start' ])
...
def reborn(self, reply, answer, name=None):
	...
	reason = locale_manager.get('user.reborn', self.lang)
	if name is not None:
